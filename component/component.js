...
function SpinnerController($rootScope) {
  ...
  // Need to destroy the on listener to avoid memory leak
  // as the rootScope will never be released unless the app exits
  ...
  $ctrl.$doCheck = function () {
    if ($ctrl.items.length !== totalItems) {
      ...
      for (var i = 0; i < $ctrl.items.length; i++) {
        ...
        promises.push(WeightLossFilterService.checkQuantity($ctrl.items[i].quantity));
      }
      ...
      .catch(function (result) {
        // Show cookie warning
        var warningElem = $element.find('div.error');
        warningElem.text(result.message);
